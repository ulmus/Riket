...
function generateRSSFeed(cfg, idx, options, limit) {
  ...
  const recentNotesText = options.rssRecentNotesText ?? "Senaste sidorna";
  const lastFewNotesText = options.rssLastFewNotesText ?? ((count) => `Senaste ${count} sidorna`);
  ...
  const description = `${limit ? lastFewNotesText(limit) : recentNotesText} på ${escapeHTML(pageTitle)}`;
