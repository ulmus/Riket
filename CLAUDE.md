...
- **Prose style:**
  ...
  - **Don't over-explain.** Trust the reader. Cut the summarizing meta-sentence that restates what a passage just showed or spells out its point (e.g. "So this is really about X, not Y" or "That is the essence of the scene"). Let the scene, rule, or example stand on its own and end on the concrete detail.
...
- **Formatting:**
  ...
  - **Images:** Embedded images (`![[Bild.png]]`) default to a smaller, right-floated thumbnail with the body text wrapping alongside them (the dossier-photo look). Place the embed just before the paragraph it illustrates. For maps, floor plans, or diagrams that need the full column width, add the `|wide` alias: `![[Karta.png|wide]]`. A fixed pixel width can still be set with a numeric alias, e.g. `![[Porträtt.png|200]]`. Every embedded image is automatically clickable on the published site to view the original at full size in an overlay (handled by `quartz/components/scripts/imageZoom.inline.ts`), so **do not** wrap embeds in a link to make them zoomable (the old `[![[Bild.png]]](<Bild.png>)` workaround) — just write the plain embed.
...
- **Publishing:** The website is built with **[Quartz 5](https://quartz.jzhao.xyz/)** (a fork-and-own static site generator vendored under `quartz/`). Published content lives in `content/`; configuration is in `quartz.config.yaml`.
  ...
  - Custom HTML pages (e.g. the print character sheet) are served from `quartz/static/` and linked like `/static/arkivet/personalakt` (Cloudflare Pages and the local dev server both serve `personalakt.html` at the extension-less URL).
  - **Personalakts-arkivet (character library):** `/static/arkivet/` is the character library/"Personalakts-arkivet" (gallery at `index.html`, editor — the *personalakt* — at `personalakt` / `personalakt.html`) with two storage backends, surfaced to players as **Skrivbordet** (this browser, `localStorage`) and **Arkivskåpet** (an optional cloud vault backed by Cloudflare Pages Functions (`functions/api/`) + a D1 database, with passwordless magic-link login (Resend)). Photos are uploaded and stored inline in each character's JSON. The frontend storage layer is `quartz/static/arkivet/store.js`. See [CHARACTER-VAULT.md](CHARACTER-VAULT.md) for the model, API and Cloudflare setup.
- **Styling:** Rely on semantic Markdown. Custom site CSS goes in `quartz/styles/custom.scss`; theme colours/fonts are in `quartz.config.yaml`. (`publish.css`, `regler.css`, `karaktär.css` are now only used by the PDF build, not the website.) Body images default to a smaller, right-aligned float so text wraps beside them; the `|wide` alias (rendered as `alt="wide"`) opts an image back into the full column for maps and diagrams. Each body image is also made clickable to open the original full size in a lightbox overlay (`#image-zoom-overlay`), wired up by `quartz/components/scripts/imageZoom.inline.ts` (registered in `quartz/plugins/emitters/componentResources.ts`) and styled in `custom.scss`.
