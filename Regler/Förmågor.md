# Närstrid
## Fäktning
*Krav:* Snabbhet 2, Strid 2
Du får +1 på slag för Snabbhet + Strid när du slåss med ett finess-vapen (närstridsvapen med egenskapen Finess).
## Slagsmål
*Krav:* Fysik 2, Strid 2
Du får +1 på slag för Fysik + Strid när du slåss obeväpnad.
## Jujutsu
Krav: Finess 2, Strid 2
Du kan använda Finess + Strid för obeväpnade närstridsattacker och får +1 på det slaget.
## Fint
*Krav:* Fäktare, Slagkämpe eller Jujutsu
Som en bihandling kan du finta en motståndare, slå för Finess + Strid. Om du lyckas så dras dina framgångar på finten av från motståndarens eventuella framgångar i dennes försvar mot en attack från dig samma runda.
## Ripost
*Krav:* Fäktning, Jujutsu
Om du blir attackerad, parerar och får minst en nivå framgång på ditt försvar så kan du spendera 1 Fokus och göra en motattack, obeväpnad eller med ett finess-vapen.
## Fällning
*Krav:* Jujutsu
Om du träffar med en obeväpnad attack så kan du spendera 1 Fokus och en Bihandling för att fälla motståndaren.
## Fastlåsning
*Krav:* Jujutsu
Om du träffar med en obeväpnad attack så kan du spendera 1 Fokus för att koppla ett grepp om personen istället för att göra skada.
## Uppercut
*Krav:* Slagsmål
Dina slagsmåls-attacker räknas som Skala 1

# Eldstrid

## Automatsalva  
_Krav:_ Kraft 2, Strid 2, vapen med egenskapen *automateld*
Du kan skjuta en lång salva med bibehållen precision

- **Huvudhandling:** Gör en avståndsattack med *Kraft + Strid*. För varje framgång utöver den första så träffar ett skott extra på målet eller på ett annat mål inom tre meter från ett tidigare träffat mål (i samma salva).
- Varje träff ger vapnets grundskada men ett lägre i skala (det vill säga en mindre skadetärning).
- Kostar 1 magasin.

## Täckande eld  
_Krav:_ Automatsalva
Du skjuter täckande eld som hindrar fienden från att förflytta sig

- **Huvudhandling:** Lägg en zon (~10 m bredd) som varar till din nästa tur. Fiender som rör sig in/igenom zonen får automatiskt en lyckad attack mot sig med en framgång och måste slå **Försvar** (normalt *Finess + Strid* och kostar en reaktion) med minst en framgång eller ta vapnets normala skada.
- Kostar **1 Fokus** och 1 magasin.

## Undertryckande eld  
_Krav:_ Täckande eld
Din täckande eld gör det också svårt att attackera

- Om du har lagt en zon med **Täckande eld** så aktiveras den även av attacker gjorda från zonen, inte bara av förflyttningar.
- Effekten av **Undertryckande eld** kommer före effekten av den utlösande attacken.
- Om den som attackerar tar skada från **Undertryckande eld** så avbryts automatiskt personens attack.

_Notis:_ Detta är en utvidgning av **Täckande eld** och använder samma zon och kostnader. Syftet är att kunna störa och avbryta fiendens attacker mot allierade genom att hålla dem under eld.

## Skarpskytt  
_Krav:_ Finess 2, Strid 2

- +1 på *Finess + Strid* med gevär.
- Ignorera -1 svårighetsgrad för långt avstånd.

## Mästerskott  
_Krav:_ Skarpskytt

- **Huvudhandling:** Spendera **1 Fokus** för att öka skalan på din träff med ett gevär med ett.

## Kallblodig träff  
_Krav:_ Mästerskott

- När du får en 12:a på en attack kan du spendera **1 Fokus** för att omedelbart göra en extra attack mot samma mål.

## Pistolskytt  
_Krav:_ Finess 2, Strid 2

- +1 på Finess + Strid med pistoler.
- Dra eller byta till pistol som gratis handling.

## Snabbskott  
_Krav:_ Pistolskytt

- **Bihandling:** Gör en extra pistolattack med -1 på slaget. Kostar **1 Fokus**.

## Skottväxling
_Krav:_ Snabbskott

- När du träffar med en pistolattack kan du spendera **1 Fokus** för att omedelbart göra en ny attack mot ett annat mål inom ~2 m från det första.
# Helande
## Terapeut
*Krav:* Samspel 2

- +1 på Samspel i terapeutiska sammanhang
- Du kan ha ett terapeutiskt samtal med en person. Slå Karisma + Samspel och minska personens stress med antalet framgångar.
# Yrken
## Fältmedicinare
*Bonus:* +1 på Intellekt eller Precision + Akademi när du ger akutvård i fält.
*Effekter:*
- 1 Fokus: Ignorera första miljö-svårigheter (mörker, kyla, press) på akutvårdsslaget.
- 1 Framgång: Patienten minskar sin Stress med 1 extra (utöver grundregeln).

## Apotekare
*Bonus:* +1 på Intellekt + Akademi för att identifiera, dosera eller framställa läkemedel och enkla antidoter.
*Effekter:*
- 1 Fokus: Ge +2 tärningar till någon annans Akutvård eller läkeslag i scenen (en gång per slag).
- 1 Framgång: Exakt dos – inga biverkningar eller förlängd verkan (välj ett).

## Radiotekniker
*Bonus:* +1 på Intellekt + Hantverk när du riggar radio/antenn eller stabiliserar förbindelse.
*Effekter:*
- 1 Fokus: Fördröj scenens nästa tidspress en runda.
- 1 Framgång: +1 steg räckvidd eller −1 steg brus (välj ett).

## Telegrafist
*Bonus:* +1 på Psyke eller Intellekt + Observation när du lyssnar i brus/störningar (morse, signalspaning).
*Effekter:*
- 1 Fokus: Den första allierade som hjälper ger +2 tärningar i stället för +1.
- 1 Framgång: Avslöja sidometadata (t.ex. sändningsfönster eller ungefärlig plats).

## Kryptograf
*Bonus:* +1 på Intellekt + Akademi vid dekryptering och kodanalys.
*Effekter:*
- 1 Fokus: Om du lyckas denna runda sänks scenens kvarvarande framgångskrav med 1.
- 1 Framgång: Få ut ett extra textsegment eller en klargörande hint.

## Förfalskare
*Bonus:* +1 på Precision + Hantverk för förfalskningar (pass, stämplar, patina).
*Effekter:*
- 1 Fokus: Första kontrollen mot dokumentet får −1 i svårighet.
- 1 Framgång: +1 på nästa relevanta Samspel-slag som använder dokumentet.

## Låssmed
*Bonus:* +1 på Intellekt eller Precision + Hantverk för att dyrka eller bypassa lås och enkla larm.
*Effekter:*
- 1 Fokus: Vid lyckat slag räknas din hjälp som +2 tärningar i stället för +1.
- 1 Framgång: Öppningen blir både tyst och snabb.

## Sappör
*Bonus:* +1 på Intellekt + Hantverk när du laddar eller riggar spräng (riktverkan, säkringar).
*Effekter:*
- 1 Fokus: Välj en skyddad zon (person eller yta) som undantas från skada vid lyckad sprängning.
- 1 Framgång: Sänk risken för larm eller oönskad uppmärksamhet ett steg.

## Mekaniker
*Bonus:* +1 på Intellekt + Hantverk för fältreparation av fordon eller maskin.
*Effekter:*
- 1 Fokus: Ge +1 tärning till nästa allierades Hantverk-slag i samma scen.
- 1 Framgång: Återfå en extra funktion eller tålighet på objektet.

## Elektriker
*Bonus:* +1 på Intellekt + Hantverk för sensorer, larm och enkla kretsar.
*Effekter:*
- 1 Fokus: Ignorera första standardtriggern på sensorn/larmet om slaget lyckas.
- 1 Framgång: Bypass utan spår – ingen logg eller avvikelse kvarlämnas.

## Maskör
*Bonus:* +1 på Karisma + Samspel för förklädnad, cover story och kroppsspråk i infiltration.
*Effekter:*
- 1 Fokus: Den första allierade som hjälper ditt sociala slag ger +2 tärningar i stället för +1.
- 1 Framgång: Få djupare åtkomstnivå eller förläng tiden innan misstanke uppstår.

## Förhörsledare
*Bonus:* +1 på Karisma + Samspel vid taktiska förhör (rapportbyggande, frågeteknik).
*Effekter:*
- 1 Fokus: Välj en fas i förhöret; +1 på den fasen även för en allierad som hjälper.
- 1 Framgång: Få en extra verifierbar uppgift utan att någon tar extra Stress.

## Officer
*Bonus:* +1 på Karisma + Strid när du leder i pågående strid (ordrar, elddisciplin).
*Effekter:*
- 1 Fokus: Ge +1 per framgång till en allierad, fler Fokus gör att fler allierade kan få samma bonus.
- 1 Fokus, 1 Bihandling: Ge en allierad +1 per framgång på initiativ, fler Fokus för att fler allierade kan få samma bonus.

## Chaufför
*Bonus:* +1 på Snabbhet + Rörlighet i biljakt och undanmanöver.
*Effekter:*
- 1 Fokus: Ge +1 tärning till nästa allierades manöver i samma jakt.
- 1 Framgång: Vinn distans eller tvinga motståndaren till ett kontrollslag.

## Underrättelseanalytiker
*Bonus:* +1 på Intellekt + Akademi eller Observation när du bygger lägesbild och källvärdering.
*Effekter:*
- 1 Fokus: Brief – lagets första Hjälpa till i nästa delscen ger +2 tärningar i stället för +1.
- 1 Framgång: Förutse nästa sannolika drag; spelledaren ger en konkret ledtråd eller varning.

## Säkerhetsofficer
*Bonus:* +1 på Intellekt + Observation för att upptäcka intrång, läckor och fällor.
*Effekter:*
- 1 Fokus: Vid lyckat slag måste motståndaren lägga en extra runda på att återta initiativ i denna delscen.
- 1 Framgång: Identifiera modus eller sannolik källa; ge +1 tärning till nästa åtgärd riktad mot samma angripare.