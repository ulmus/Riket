## Akutvård 

**Syfte:** Stoppa döende, blödning och chock; skapa förutsättningar för fortsatt vård.
- **När:** inom minuter efter skadan (samma scen).
- **Slå för:** **Intellekt + Hantverk**, svårighet 0 om minst Sår; -1 om minst Kritiskt Sår; -2 om minst Dödligt Sår.
- **Utrustning:** +1 tärning med första-hjälpen-kit; +2 tärningar med läkarväska.

**Varje framgång kan användas till:**
* Göra ett dödligt sår ett steg mindre dödligt (Rundor → Minuter → Timmar → Stabilt). Det behåller värdet på T12:an som slogs för dödlighet, men ökar en enhet.
* Ignorera det allmänna avdraget av en skada (Sår, Kritiskt Sår eller Dödligt Sår) 
* Återfå en Stress som tagits från skada
        
**Begränsning:** Akutvård per patient **en gång per scen** och inte igen innan personen skadats igen.

## Läkning
Tiden för naturlig läkning beror på skadans nivå, omständigheter och personens läkekött. 
### Normala läketider
- **Sår** – 1 vecka till Oskadd
- **Kritiskt Sår** – 4 veckor till Sår
- **Dödligt Sår** – 4 veckor till Kritiskt Sår

### Läkeslag
När scenen är över så kan du slå ett läkeslag (endast **Fysik**) med följande modifikationer:
- **Kritiskt sårad** -1
- **Dödligt sårad** -2
- **Vila** +2
- **Fält/hemvård** +2
- **Sjukhus** +4
- **Aktiv vård**, vårdare slår för Intellekt + Akademi (Medicin), framgångar ger bonus (endast en vårdgivare per läkeslag)

Om det blir minst en framgång på läkeslaget så tillstöter inga komplikationer. Varje ytterligare framgång sänker tiden med en, eller om läketiden redan är en vecka, med en dag. Du slår ett läkeslag per skada, men de läker parallellt.

Om du börjar med minst ett Kritiskt eller Dödligt Sår så kommer du få slå läkeslag igen när de har konverterats ner i allvarlighetsgrad.

Nya skador innebär också nytt läkeslag. De nya såren läkare parallellt med de gamla.
### Komplikationer
Om läkeslaget misslyckas riskerar det att bli komplikationer, slå på komplikationstabellen, +1 om du är kritiskt sårad, +2 om du är dödligt sårad.

| 1D12+mod | Komplikation                                                                                                                                                                                                                                                      |
| -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1-6      | Ingen                                                                                                                                                                                                                                                             |
| 7-8      | Lätt infektion, öka läketiden med en vecka                                                                                                                                                                                                                        |
| 9-10     | Kritiskt tillstånd, om du är under vård, så får vårdaren ett slag för Intellekt + Akademi (Medicin), om misslyckat så uppgraderas din värsta skada ett steg. Om du inte är under vård så sker det automatiskt. Om din värsta skada var ett Dödligt Sår så dör du. |
| 11-12    | Svår infektion, din värsta skada uppgraderas ett steg, om din värsta skada var Dödligt Sår så dör du                                                                                                                                                              |
| 13-14    | Permanent skada, du får ett permanent avdrag på -1 på Rörlighets-slag på grund av smärta                                                                                                                                                                          |

