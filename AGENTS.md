...
- **Publishing:** The website is built with **[Quartz 5](https://quartz.jzhao.xyz/)** (a fork-and-own static site generator vendored under `quartz/`). Published content lives in `content/`; configuration is in `quartz.config.yaml`.
  ...
  - **Character vault:** the sheet has an optional server-side save/load feature backed by Cloudflare Pages Functions (`functions/api/`) and a D1 database. It is passwordless (magic-link email via Resend). See [CHARACTER-VAULT.md](CHARACTER-VAULT.md) for the architecture, API and Cloudflare setup. The frontend lives in `quartz/static/charsheet/vault.js`.
