// Transactional email via Resend (https://resend.com). The sender (MAIL_FROM)
// must be on a domain you have verified in Resend.

const DEFAULT_FROM = "I Rikets Tjänst <noreply@exostra.se>";

// Shared "T-Kontoret" email chrome; `inner` is the message-specific body.
function shell(inner) {
  return (
    `<!doctype html><html lang="sv"><body style="margin:0;background:#2b2a26;padding:32px 16px;font-family:Arial,Helvetica,sans-serif;color:#23201a;">
...
// The shared "Logga in" call-to-action button.
function loginButton(link) {
  return `<p style="margin:0 0 22px;"><a href="${link}" style="display:inline-block;background:#0c3a54;color:#f3ecdb;text-decoration:none;font:700 13px/1 Arial;letter-spacing:.04em;text-transform:uppercase;padding:13px 22px;border-radius:4px;">Logga in</a></p>`;
}

// POST one email to Resend. Throws an Error (with `.status`) on failure so the
// caller can surface the provider's status code. The most common cause is a
// `from` address whose domain isn't verified in Resend.
async function send(env, { to, subject, html, text }) {
  ...
  if (!res.ok) {
    ...
    // Surface the real reason in the deployment's function logs.
...
export async function sendMagicLink(env, email, link) {
  ...
  await send(env, { to: email, subject: "Din inloggningslänk till Rollpersonsvalvet", html, text });
}

/** Invitation to join someone's vault. `link` is a magic-link login URL. */
export async function sendInvite(env, email, link, inviterEmail) {
  const text =
    "Hej!\n\n" +
    inviterEmail +
    " har bjudit in dig till sitt rollpersonsvalv i I Rikets Tjänst och kan nu dela " +
    "rollpersoner med dig. Klicka på länken för att logga in (inget lösenord behövs); " +
  ...
  const html = shell(
    `<h1 style="margin:6px 0 14px;font:700 22px/1.1 Arial;color:#23201a;">Du har bjudits in</h1>
  ...
  await send(env, { to: email, subject: `${inviterEmail} har delat rollpersoner med dig`, html, text });
}
